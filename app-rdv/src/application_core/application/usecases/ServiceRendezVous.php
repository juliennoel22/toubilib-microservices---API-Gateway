...
class ServiceRendezVous implements ServiceRendezVousInterface
{
    ...
    {
        error_log("Service: ListerRDVID called for " . $pid);
        $rdvs = $this->rendezVousRepository->GetRdvsbyPrat($pid);
        error_log("Service: Repo returned " . count($rdvs) . " rdvs");
        $rdv = [];
        foreach ($rdvs as $element) {
            try {
                $dto = new RendezVousDTOID(
                    $element->getId(),
                ...
                error_log("Service: Loop Error: " . $e->getMessage());
                throw $e;
            }
        }
        error_log("Service: Returning DTOs");
        return $rdv;
    }
