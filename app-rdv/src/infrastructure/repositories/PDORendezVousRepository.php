...
class PDORendezVousRepository implements RendezVousRepositoryInterface
{
    ...
    public function getAllRendezVous(): array
    {
        ...
        foreach ($results as $rt) {
            try {
                ...
                // error_log("Repo: Error fetching praticien: " . $e->getMessage());
    ...
    public function GetRdvsbyPrat(string $pid): array
    {
        ...
        $pdopraticien = null;
        try {
            $pdopraticien = $this->praticien_interface->findPraticienId($pid);
        } catch (\Throwable $e) {
            $pdopraticien = new Praticien($pid, 'Inconnu', '', '', '', new Specialite('', '', ''));
        }
